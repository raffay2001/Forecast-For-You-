    ...
    # User is not authenticated
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(
                    request, f"Account for {user_name} has been made successfully, You can now log in")
    ...
    # User is not authenticated
        ...
        if request.method == "POST":
            ...
            # if the user enters wrong credenials then
                ...
                error = True
                messages.error(
                    request, f"Incorrect Username or Password, Try Again.")
                context = {'error': error}
...
# FUNCTION FOR THE FORECAST PAGE
def forecast(request):
    if request.method == 'POST':
        city = request.POST['city']
        if city == '':
            return redirect('application:forecast')
        else:
            try:
                source = urllib.request.urlopen(
                    'http://api.openweathermap.org/data/2.5/weather?q='+city+'&units=metric&appid=8983c52142955c38d01ff0369ef00cd5').read()
                ...
                return render(request, 'detailed_forecast.html', data)
